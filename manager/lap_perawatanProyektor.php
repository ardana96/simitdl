<?php
require('kop_perawatanProyektor.php');
...
$pdf->SetWidths(array(7,25,20,25,37,40,15,19,16,20,20,13,13,15));
...
//$sql=mysql_query("Select * from peripheral where (bulan LIKE '%".$bulan."%' OR '".$bulan."' = '') AND (divisi LIKE '%".$pdivisi."%' OR '".$pdivisi."' = '') and tipe = 'proyektor'  order by nomor ");
...
$sql=mysql_query("SELECT 
    ...
    MAX(CASE WHEN d.nama_perawatan = 'Kondisi Fisik Proyektor' THEN 'true' END) AS item2,
    MAX(CASE WHEN d.`nama_perawatan` = 'Kondisi Filter Udara' THEN 'true' END) AS item3,
    MAX(CASE WHEN d.`nama_perawatan` = 'Kondisi Lensa' THEN 'true' END) AS item4,
	MAX(CASE WHEN d.`nama_perawatan` = 'Kondisi Pencahayaan' THEN 'true' END) AS item5
...
$data = array(
	...
	array($no++, $lokasi,$tgl_perawatan,$tanggal_realisasi ,$id_perangkat,$perangkat.' / '.$user,$item1, $item2, $item3, $item4, $item5, $treated_by , $approve_by,'')
